# Changelog — universe-pointset

Formato: [Keep a Changelog](https://keepachangelog.com/pt-BR/1.1.0/).
SemVer. Versionamento independente do core (ver `docs/UNIVERSE-VERSIONING.md`).

## [1.0.0] — 2026-05-20

### Added
- Migração inicial para WASM via universe-sdk ABI v1 (parte de YG-57).
- Sandbox geométrico de pontos.

### Changed
- Versionamento desacoplado do core a partir de YG-64 (era `0.1.0` literal).

### Note
- Descontinuado como universo no catálogo `/api/v1/universos` (decisão de
  produto); o crate permanece versionado para histórico e reuso.
