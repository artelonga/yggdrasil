...
/// Response body do `POST /api/v1/auth/verify` em caso de sucesso.
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Código de acesso enviado por e-mail (ou silenciado por rate-limit)")
    ),
    tag = "auth"
)]
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "JWT emitido com sucesso", body = TokenResponse),
        (status = 410, description = "Código expirado ou não encontrado"),
        (status = 422, description = "Código incorreto")
    ),
    tag = "auth"
)]
